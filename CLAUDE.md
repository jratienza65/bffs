...
- An overlay's lines wrap rather than truncate (`wrapAll` in `ops.go`), and an overlay below the 96-column breakpoint is sized to the frame, because `workspace.View` draws it there alone. A key press stops wheel momentum for 350 ms (`momentumWindow`), so `esc` interrupts a touchpad flick. `y` copies what an overlay shows; `c` copies the pairing code while a send waits.
- Rendering has its own harness: 24 golden frames under `internal/tui/testdata` (`make golden` rewrites them — the diff is the review), a sweep of every state at twelve sizes, a rung table of what each token becomes at 256 and 16 colours, a scan for holes in a painted bar, an ASCII sweep for `BFFS_ASCII`, benchmarks for the wheel path, and `tools/drive.py` for a real pty. `BFFS_DEBUG=<file>` logs one line per event.
