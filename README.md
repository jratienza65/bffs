...
```
$ bffs trust
project:  ~/build/projects/bffs        (key: /Users/dev/build/projects/bffs)

ACCOUNT      FOLDER-TRUST  EXTERNAL-IMPORTS  TOOLS  MCP
* alice     accepted      allowed           3      2 enabled
  bob   -             -                 -      -
  bob2  -             declined          -      -
...
```
$ bffs trust sync --to bob
project ~/build/projects/bffs → account "bob" (from "alice"):
...
apply to ~/Library/Application Support/bffs/sessions/bob/.claude.json? [y/N] y
...
`w` opens the transfer wizard: send or receive; how much — the sessions you marked, one project, or every project of the pool; what exactly — a checklist of that project's sessions and memory files, or of the pool's projects (`space` toggles one, `a` a whole section; sessions you marked arrive pre-checked), plus tool results, file history, prompt history and live sessions; and how — over the local network with a pairing code, to a `.bffs` file, or a one-line ssh pipe it shows for you to run — or where a received bundle comes from (a machine running the serve, or a `.bffs` file, whose manifest is reviewed and placed the same way). Below 96 columns the panels take the width and `enter` shows the preview; `esc` always goes back. Colours come from a theme — `default`, `catppuccin`, `gruvbox`, `tokyo-night`, `solarized` or `mono` — with light and dark variants picked from your terminal's background; `T` cycles them and remembers the choice in `state.toml`, `BFFS_THEME=<name>` overrides it and `NO_COLOR` forces mono. Every coloured signal keeps its glyph or word, so mono loses nothing. The mouse works too: click a panel to focus it and pick the row under the pointer, click the preview to scroll it with the keys, and use the wheel over whichever pane the pointer is on, which scrolls the view without moving the selection (clicks also choose in the menu and the wizard). Drag over the preview, a transcript or a diff to select text — releasing copies it, `y` copies again and `esc` lets go; on an action screen (a confirmation, a receipt, the serve banner) `y` copies what it shows, and `c` copies the pairing code while a send is waiting — or hold shift for your terminal's own selection. Paths in a preview are links your terminal opens on cmd- or ctrl-click, and `y` copies the row's path. Memory files are rendered as the Markdown they are, `D` shows what differs between two copies of one, and `BFFS_ASCII=1` swaps every symbol for an ASCII stand-in. Every action stays reachable from the keyboard. Keys: `1`–`3` or `tab`/`h`/`l` move between panels, `/` filters, `x` opens the action menu, `?` lists every key, `+`/`_` force the preview or the panels. `n` on the accounts panel adds an account — a Claude subscription (it hands the terminal to `claude auth login` and records the account when it returns) or an `sk-ant-…` key, typed without echo. Actions run the same engines as the commands: `e` export to a file, `s` send over the LAN, `i` receive, `c` copy to another account, `r` rehome, `R` resume in claude, `t` trust matrix, `S` sync the project's memory into a full-isolation account, `L` point an account's last session at the selected one, `p` scan memory paths. Nothing in the browser deletes; that stays `bffs sessions rm`.
...
- `--map OLD=NEW` (repeatable) is a prefix rule: sessions and memory recorded
  ...
  longest matching rule wins, so `--map /Users/dev=/home/dev` covers every
...
```
$ bffs copy --from alice --to bob --project .
nothing to copy: "alice" and "bob" share one projects pool (partial isolation) — the transcripts and the memory
for ~/build/projects/bffs are already the same files. What differs per account is trust:
    bffs trust sync --from alice --to bob --project /Users/dev/build/projects/bffs
```
...
Exporting from the shared pool (~/.claude; partial isolation: alice, bob):
...
Bundle 6f1e2c0a from mac-a (dev, darwin/arm64, bffs 0.3.0, claude 2.1.259, account "alice", partial):
  project /Users/dev/build/projects/bffs        exists here ✓ (same directory — no rehome needed)
    12 sessions  148.2 MB   memory 6 files   (trust: accepted on mac-a — informational)
    note: memory files in this bundle will be loaded into every future claude session for /Users/dev/build/projects/bffs
...
Import into ~/.claude? [y/N] y
  ...
  sessions   12 committed to projects/-Users-dev-build-projects-bffs/; 0 skipped
  memory     6 files into ~/.claude/projects/-Users-dev-build-projects-bffs/memory (side files: *.imported-6f1e2c0a.md; MEMORY.md untouched)
  history    41 prompt lines added
Done in 2.8s. Check it:

    claude --resume 1e005053-380a-4245-a145-52c2715afa73
    bffs sessions list --project /Users/dev/build/projects/bffs
