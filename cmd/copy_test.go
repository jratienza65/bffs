...
func TestRenderNothingToCopy(t *testing.T) {
	...
	renderNothingToCopy(&sb, "alice", "bob", filepath.Join(home, "build", "projects", "bffs"))
	out := sb.String()
	for _, want := range []string{
		...
		"    bffs trust sync --from alice --to bob --project " + filepath.Join(home, "build", "projects", "bffs"),
	} {
...
func TestRunCopyNothingToCopy(t *testing.T) {
	...
	if err := runCopy(c, f.cfgDir, pr, f.request("alice", "bob", false), false); err != nil {
		t.Fatalf("shared pool must be an answer, not an error: %v", err)
	}
	if !strings.Contains(out.String(), "nothing to copy") || !strings.Contains(out.String(), "bffs trust sync --from alice --to bob") {
		t.Errorf("output:\n%s", out.String())
	}
}
