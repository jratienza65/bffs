...
func testManifest(home string) *bundle.Manifest {
	...
	m := &bundle.Manifest{
		...
		Source: bundle.Source{Hostname: "mac-a", User: "dev", Home: "/Users/dev", OS: "darwin", Arch: "arm64", Account: "alice", AccountType: "oauth", Isolation: "partial"},
...
func TestRenderExportSummary(t *testing.T) {
	...
	for _, want := range []string{
		"Exporting from the shared pool (" + filepath.Join("~", ".claude") + "; partial isolation: alice, bob):",
...
func TestExportRootLabel(t *testing.T) {
	...
	cases := []struct {
		...
		{sharedRoot(home), "the shared pool (" + filepath.Join("~", ".claude") + "; partial isolation: alice, bob)"},
...
func TestRenderImportsTable(t *testing.T) {
	...
	recs := []imports.Record{
		{BundleID: "aaaaaaaa-0000-4000-8000-000000000001", Kind: imports.KindImport, ImportedAt: catalogNow.Add(-30 * 24 * time.Hour), Account: "work",
			Source:   imports.Source{Hostname: "mac-old", User: "dev"},
		...
		{BundleID: testBundleID, Kind: imports.KindImport, ImportedAt: catalogNow.Add(-8 * 24 * time.Hour),
			Source:   imports.Source{Hostname: osc52 + "mac-a", User: "dev"},
	...
	newest := lines[1]
	for _, want := range []string{"mac-a (dev)", "home", "2026-08-16 (8d ago)", "2 (+1 skipped)"} {
