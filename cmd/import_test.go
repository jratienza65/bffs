...
func TestParseFrom(t *testing.T) {
	...
	cases := []struct {
		...
		{filepath.Join(dir, "missing.bffs"), 0, "", fmt.Sprintf("bundle file %q not found", filepath.Join(dir, "missing.bffs"))},
		{"missing.BFFS", 0, "", `bundle file "missing.BFFS" not found`},
		{subdir, 0, "", fmt.Sprintf("bundle %q is a directory", subdir)},
		{"", 0, "", "--from is required"},
...
func TestRenderImportSummary(t *testing.T) {
	...
	for _, want := range []string{
		...
		"    note: memory files in this bundle will be loaded into every future claude session for projects/-Users-dev-build-projects-bffs/memory (as-is)",
	...
	sb.Reset()
	renderImportSummary(&sb, sum)
	if !strings.Contains(sb.String(), "imported as-is under projects/-Users-dev-build-projects-bffs/ (--as-is); rehome later") {
...
func TestRenderImportReceipt(t *testing.T) {
	...
	rep := porter.Report{
		...
		SweptCount:   1,
		HistoryLines: 41,
		Verify:       []string{"cd /home/dev/src/bffs && claude --resume " + testSID1, osc52 + "claude --resume " + testSID2},
	}
	rep.MemoryReview = map[string]int{rep.MemoryDirs[0]: 3}
	rec := &imports.Record{BundleID: testBundleID, Sessions: []imports.Session{
		{ID: testSID1, Slug: "-home-dev-src-bffs", NewCwd: "/home/dev/src/bffs", Status: imports.StatusPlaced},
		{ID: testSID2, Slug: "-Users-dev-old", Status: imports.StatusPending},
		{ID: testSID3, Slug: "-home-dev-src-bffs", Status: imports.StatusSkipped},
	}, Memories: []imports.Memory{{Dir: rep.MemoryDirs[0], OldCwd: osc52 + "/home/dev/src/my bffs", Status: imports.StatusPlaced}}}
	...
	out := sb.String()
	for _, want := range []string{
		"  sessions   2 committed to projects/-Users-dev-old/ (as-is, pending rehome), projects/-home-dev-src-bffs/; 2 skipped; 1 existing set aside as *.bffs-replaced-<time> (never deleted); 1 mtime raised to the retention floor",
		...
		"\n    cd /home/dev/src/bffs && claude --resume " + testSID1,
		"\n    claude --resume " + testSID2,
		"\n    bffs sessions list --project /home/dev/src/bffs\n",
		"\n    bffs memory scan-paths --project '/home/dev/src/my bffs'   # 3 lines to review\n",
...
func TestFetchPrinterLines(t *testing.T) {
	...
	p.event(transfer.Event{Kind: "code-ok", Peer: "192.168.1.20:7345", Text: "mac-a accepted the code (bffs 0.3.0, user dev, account alice, compression 1)"})
