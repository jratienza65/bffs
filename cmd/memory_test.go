...
func testMemories(home string) []transcripts.Memory {
	...
	return []transcripts.Memory{
		{
			Root: shared, Slug: "-Users-dev-build-projects-bffs", Dir: filepath.Join(shared.Dir, "-Users-dev-build-projects-bffs", "memory"),
			...
			Files: []transcripts.MemoryFile{
				...
				{Name: "notes.md", Size: 3_400, ModTime: catalogNow.Add(-26 * time.Hour), Pinned: true, AbsolutePaths: []string{"/Users/dev/x", "/Users/dev/y"}},
				{Name: "logs/2026/08/24/a.md", Size: 10, ModTime: catalogNow.Add(-3 * time.Hour), AbsolutePaths: []string{"/Users/dev/x"}},
			},
		},
		{
...
func TestRenderMemoryShow(t *testing.T) {
	...
	for _, want := range []string{
		...
		"memory:   " + filepath.Join("~", ".claude", "projects", "-Users-dev-build-projects-bffs", "memory") + "   (shared pool — visible to: alice, bob)",
...
func TestRenderScanPaths(t *testing.T) {
	...
	refs := []transcripts.PathRef{
		{File: "notes.md", Line: 3, Path: "/Users/dev/x", Kind: transcripts.PathKindAbs},
	...
	out := sb.String()
	for _, want := range []string{
		"~/.claude/projects/-a/memory/notes.md:3: /Users/dev/x\n",
...
func TestSelectMemories(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(memDir, "MEMORY.md"), []byte("- [Notes](notes.md) - see /Users/dev/x and @~/foo.md\n"), 0o600); err != nil {
		t.Fatal(err)
	}
	...
	if out := sb.String(); !strings.Contains(out, "MEMORY.md:1: /Users/dev/x") || !strings.Contains(out, "@ref MEMORY.md:1: @~/foo.md") {
