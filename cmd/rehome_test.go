...
func TestRenderRehomePlan(t *testing.T) {
	...
	p := rehome.Plan{
		Root:     root,
		Mappings: []rehome.Mapping{{Old: "/old", New: "/home/d/src"}},
		Moves: []rehome.Move{
			{SessionID: rehomeSidOld, From: "/x/projects/-old/" + rehomeSidOld + ".jsonl", NewSlug: "-home-d-src", NewCwd: "/home/d/src", OldCwd: "/old", Title: "Fix " + osc52 + "shim", LastTS: older, Sidecar: true},
			{SessionID: testSID2, From: "/x/projects/-old/" + testSID2 + ".jsonl", NewSlug: "-home-d-src", NewCwd: "/home/d/src", OldCwd: "/old", Title: "Newer", LastTS: older.Add(time.Hour), SameSlug: true},
		},
		Memory: []rehome.MemoryMove{{From: "/x/projects/-old/memory" + osc52, To: "/x/projects/-home-d-src/memory", OldCwd: "/old"}},
	...
	for _, want := range []string{
		...
		"projects/-old → projects/-home-d-src + sidecar",
		"projects/-home-d-src (already there; relocated stamp only)",
		"memory  /x/projects/-old/memory → /x/projects/-home-d-src/memory  (merge; the old directory stays)",
		...
		"set     lastSessionId for /home/d/src → b19c4e20 in /x/.claude.json (claude --continue there opens it)",
...
func TestRenderRehomeResult(t *testing.T) {
	...
	res := rehome.Result{
		...
		Verify:  []string{"cd /home/d/src && claude --resume " + rehomeSidOld},
	}
	var sb strings.Builder
	renderRehomeResult(&sb, res)
	got := sb.String()
	for _, want := range []string{
		"moved 1 session → projects/-home-d-src (relocated stamp appended; mtimes and picker order preserved)\n",
		"1 session not moved (see the error)\n",
		"merged memory: 3 files (1 new, 1 identical, 1 renamed c.imported-6f1e2c0a.md); MEMORY.md +1 section; rewrote /old → /home/d/src in 1 file\n",
		"  note: MEMORY.md is 300 lines\n",
		"1 memory line still mentions absolute paths: bffs memory scan-paths --project /home/d/src\n",
		"2 sessions not moved: 1 held by a running claude, 1 refused (listed above)\n",
		"verify: cd /home/d/src && claude --resume " + rehomeSidOld + "\n",
	} {
		if !strings.Contains(got, want) {
			t.Errorf("output lacks %q:\n%s", want, got)
		}
	...
	res = rehome.Result{
		Plan: rehome.Plan{
			Root:     root,
			Mappings: []rehome.Mapping{{Old: "/old", New: "/home/d/src"}, {Old: "/other", New: "/home/d/other"}},
			Moves:    []rehome.Move{{SessionID: rehomeSidOld, NewSlug: "-home-d-src", NewCwd: "/home/d/src", OldCwd: "/old"}, {SessionID: rehomeSidNew, NewSlug: "-home-d-src", NewCwd: "/home/d/src", OldCwd: "/old"}},
			Memory: []rehome.MemoryMove{
				{From: "/x/projects/-old/memory", To: "/x/projects/-home-d-src/memory", OldCwd: "/old", Mode: rehome.MemorySkip, Unchanged: []string{"a.md", "b.md"}},
				{From: "/x/projects/-other/memory", To: "/x/projects/-home-d-other/memory", OldCwd: "/other", Mode: rehome.MemoryOverwrite, Added: []string{"MEMORY.md", "z.md"}, Remaining: []transcripts.PathRef{{File: "z.md", Line: 1, Path: "/opt/x"}, {File: "z.md", Line: 2, Path: "/opt/y"}}},
			},
		},
		Moved:          []string{rehomeSidOld, rehomeSidNew},
		LastSessionSet: rehomeSidNew,
		Verify:         []string{"cd /home/d/src && claude --resume " + rehomeSidNew},
	}
	sb.Reset()
	renderRehomeResult(&sb, res)
	got = sb.String()
	for _, want := range []string{
		"moved 2 sessions → projects/-home-d-src (relocated stamp appended; mtimes and picker order preserved)\n",
		"kept memory for /home/d/src: 2 files (2 identical, --memory skip)\n",
		"replaced memory for /home/d/other: 2 files (2 new, the previous directory set aside, never deleted)\n",
		"2 memory lines still mention absolute paths: bffs memory scan-paths --project /home/d/other\n",
		"lastSessionId → 0c5e19b2 (claude --continue opens it)\n",
		"verify: cd /home/d/src && claude --resume " + rehomeSidNew + "\n",
	} {
		if !strings.Contains(got, want) {
			t.Errorf("output lacks %q:\n%s", want, got)
		}
