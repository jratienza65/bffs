...
func testSessions(home string) []transcripts.Session {
	...
	slug := "-Users-dev-build-projects-bffs"
	rec := &imports.Record{BundleID: "6f1e2c0a-0000-4000-8000-000000000001", Kind: imports.KindImport, ImportedAt: catalogNow.Add(-8 * 24 * time.Hour), Account: "bob",
	...
	return []transcripts.Session{
		{
			...
			Account: "alice", AttribSource: "launch-log",
		},
		{
			...
			Cwd: "/home/dev/src/bffs", HeadCwd: "/home/dev/src/bffs",
			GitBranch: "main", LastTS: catalogNow.Add(-9 * 24 * time.Hour), Size: 400_000,
			Import: &imports.SessionRef{Record: rec, Session: &imports.Session{ID: testSID2, OldCwd: "/home/dev/src/bffs", Status: imports.StatusPending, GitRemote: "git@github.com:x/bffs.git"}},
		},
	}
}
...
func TestRenderSessionsTable(t *testing.T) {
	...
	for _, want := range []string{
		"project " + filepath.Join("~", "build", "projects", "bffs") + "  (shared pool: alice, bob)",
		"ID", "TITLE", "ACCOUNT", "LAST", "SIZE", "BRANCH", "STATE",
		"1e005053", "Plan: session export", "alice", "2h ago", "42.6 MB", "main", "live",
...
func TestRenderSessionsTableGroupsProjects(t *testing.T) {
	home := fakeHome(t)
	ss := testSessions(home)
	ss[1].Slug = "-home-dev-src-bffs"
	ss = append(ss, transcripts.Session{ID: testSID3, Slug: "-home-dev-src-bffs", Root: sharedRoot(home), LastTS: catalogNow.Add(-time.Hour), Size: 10})
	...
	if !strings.Contains(out, "project /home/dev/src/bffs  (account: work)") {
...
func TestRootLabel(t *testing.T) {
	...
	cases := []struct {
		...
		{sharedRoot(home), "shared pool: alice, bob"},
...
func TestSessionInfoJSONShape(t *testing.T) {
	...
	if imp.BundleID != "6f1e2c0a-0000-4000-8000-000000000001" || imp.OldCwd != "/home/dev/src/bffs" || imp.OldHost != "mac-a" || imp.OldHome != "/Users/dev" || imp.GitRemote != "git@github.com:x/bffs.git" || imp.FirstAt != "" {
...
func TestRenderSessionShow(t *testing.T) {
	...
	for _, want := range []string{
		...
		"root:         " + filepath.Join("~", ".claude", "projects") + "  (shared pool: alice, bob)",
		"cwd:          " + filepath.Join("~", "build", "projects", "bffs") + "  (exists)",
		"account:      alice  (launch-log)",
		"state:        live (pid 38445)",
		"last-session pointer: alice (claude-recorded)",
		"size:         42.6 MB",
		"branch:       main",
		"version:      2.1.259",
		"transcript:   " + filepath.Join("~", ".claude", "projects", "-Users-dev-build-projects-bffs", testSID1+".jsonl"),
		"(1.2 MB, 2 subagents)",
