...
// twoAccountEnv builds a trustEnv with oauth accounts alice (active) and
// bob plus home, all files under temp dirs, nothing written yet.
func twoAccountEnv(t *testing.T) *trustEnv {
	t.Helper()
	home := setHome(t)
	cfg := t.TempDir()
	accs := store.Accounts{Accounts: map[string]store.Account{
		"alice": {Type: store.TypeOAuth},
		"bob":   {Type: store.TypeOAuth},
	...
	return &trustEnv{cfgDir: cfg, homeJSON: files[trust.HomeName], accs: accs, files: files, activeRow: "alice"}
}
...
func sampleBlock(key string) trustBlock {
	return trustBlock{
		...
		Statuses: []trust.Status{
			{Account: "alice", ProjectKey: key, Present: true, Folder: trust.Accepted, External: trust.Accepted, Tools: 3, MCPEnabled: 2},
			{Account: "bob", ProjectKey: key},
			{Account: "bob2", ProjectKey: key, Present: true, Folder: trust.Inherited, InheritedFrom: filepath.Dir(key), External: trust.Declined},
...
func TestRenderTrustMatrix(t *testing.T) {
	...
	renderTrustMatrix(&sb, sampleBlock(key), "alice")
	out := sb.String()
	...
	for _, want := range []string{
		...
		"* alice", "accepted", "allowed", "2 enabled",
	...
	for _, line := range strings.Split(out, "\n") {
		if strings.HasPrefix(line, "  bob ") {
			...
			if f := strings.Fields(line); len(f) != 5 || f[1] != "-" || f[2] != "-" || f[3] != "-" || f[4] != "-" {
				t.Errorf("bob row = %q, want four dashes", line)
			}
		}
	}
	if !found {
...
func TestRenderTrustMatrixLiveOwnerAndNothingMissing(t *testing.T) {
	...
	b := trustBlock{
		Key: key,
		Statuses: []trust.Status{
			{Account: "alice", Present: true, Folder: trust.Accepted, External: trust.Accepted},
			{Account: trust.HomeName, Present: true, Folder: trust.Accepted, External: trust.Accepted},
		},
		Live: []liveLine{{PID: 7, Cwd: key, Owner: "alice"}},
	}
	var sb strings.Builder
	renderTrustMatrix(&sb, b, trust.HomeName)
	out := sb.String()
	if !strings.Contains(out, "live: pid 7 in "+filepath.Join("~", "p")+` (account "alice")`) {
...
func TestRenderTrustJSON(t *testing.T) {
	...
	rows := got[0].Accounts
	if len(rows) != 4 || rows[0].Account != "alice" || rows[0].Folder != "accepted" || rows[0].External != "accepted" || rows[0].Tools != 3 || rows[0].MCPEnabled != 2 {
		t.Errorf("alice row = %+v", rows[0])
	}
	if rows[1].Folder != "unset" || rows[1].Present {
		t.Errorf("bob row = %+v", rows[1])
	}
	if rows[2].Folder != "inherited" || rows[2].InheritedFrom != filepath.Dir(key) || rows[2].External != "declined" {
		t.Errorf("bob2 row = %+v", rows[2])
	}
...
func TestTrustSyncTargets(t *testing.T) {
	env := twoAccountEnv(t)
	all, err := trustSyncTargets(env, "alice", trustSyncAll)
	if err != nil {
		t.Fatal(err)
	}
	if want := []string{"bob", trust.HomeName}; strings.Join(all, ",") != strings.Join(want, ",") {
	...
	if _, err := trustSyncTargets(env, "alice", "alice"); err == nil || !strings.Contains(err.Error(), "nothing to copy") {
...
// syncFixture seeds alice as an accepting source and returns the project
// key; bob and home start with no file at all.
func syncFixture(t *testing.T, env *trustEnv) string {
	t.Helper()
	key := projectDir(t)
	writeClaudeJSON(t, env.files["alice"], map[string]any{key: acceptedEntry()})
	return key
}
...
func TestTrustSyncConfirmYes(t *testing.T) {
	...
	err := runTrustSync(c, pr, env, syncRequest{To: []string{"bob"}, Keys: []string{key}}, true)
	if err != nil {
		t.Fatalf("runTrustSync: %v", err)
	}
	for _, want := range []string{
		`project ` + key + ` → account "bob" (from "alice"):`,
		"hasTrustDialogAccepted", "unset -> true",
		"hasClaudeMdExternalIncludesApproved",
		"hasClaudeMdExternalIncludesWarningShown",
		"apply to " + env.files["bob"] + "? [y/N] ",
		`updated 1 project in "bob". A running claude on that account picks the change up within about a second.`,
	...
	got := readProjectsLoose(t, env.files["bob"])[key]
	...
	if _, ok := got["lastSessionId"]; ok && got["lastSessionId"] == "sid-alice" {
...
func TestTrustSyncConfirmNo(t *testing.T) {
	...
	if err := runTrustSync(c, pr, env, syncRequest{To: []string{"bob"}, Keys: []string{key}}, true); err != nil {
		t.Fatalf("runTrustSync: %v", err)
	}
	if !strings.Contains(out.String(), "aborted") {
		t.Errorf("declining should print aborted:\n%s", out.String())
	}
	if _, err := os.Stat(env.files["bob"]); !os.IsNotExist(err) {
...
func TestTrustSyncNonInteractiveNeedsYes(t *testing.T) {
	...
	key := syncFixture(t, env)
	c, pr, _ := newTestCmd("")
	err := runTrustSync(c, pr, env, syncRequest{To: []string{"bob"}, Keys: []string{key}}, false)
	...
	c, pr, out := newTestCmd("")
	if err := runTrustSync(c, pr, env, syncRequest{To: []string{"bob"}, Keys: []string{key}, Yes: true}, false); err != nil {
		t.Fatalf("with -y: %v", err)
	}
	if strings.Contains(out.String(), "[y/N]") {
		t.Errorf("-y must not prompt:\n%s", out.String())
	}
	if readProjectsLoose(t, env.files["bob"])[key]["hasTrustDialogAccepted"] != true {
...
func TestTrustSyncDryRun(t *testing.T) {
	...
	key := syncFixture(t, env)
	c, pr, out := newTestCmd("y\n")
	if err := runTrustSync(c, pr, env, syncRequest{To: []string{"bob", trust.HomeName}, Keys: []string{key}, DryRun: true}, false); err != nil {
		t.Fatalf("dry run: %v", err)
	}
	if !strings.Contains(out.String(), "dry run: nothing written") || strings.Contains(out.String(), "[y/N]") {
		t.Errorf("dry run output:\n%s", out.String())
	}
	for _, name := range []string{"bob", trust.HomeName} {
...
func TestTrustSyncNoChangesAndNoSource(t *testing.T) {
	...
	writeClaudeJSON(t, env.files["bob"], map[string]any{key: acceptedEntry()})
	c, pr, out := newTestCmd("")
	if err := runTrustSync(c, pr, env, syncRequest{To: []string{"bob"}, Keys: []string{key}}, false); err != nil {
	...
	other := projectDir(t)
	c, pr, _ = newTestCmd("")
	err := runTrustSync(c, pr, env, syncRequest{To: []string{"bob"}, Keys: []string{other}}, false)
	...
	c, pr, out = newTestCmd("")
	if err := runTrustSync(c, pr, env, syncRequest{From: "alice", To: []string{"bob"}, Keys: []string{other}}, false); err != nil || !strings.Contains(out.String(), "no changes") {
...
func TestTrustSyncIncludePermissions(t *testing.T) {
	...
	key := syncFixture(t, env)
	c, pr, out := newTestCmd("y\n")
	if err := runTrustSync(c, pr, env, syncRequest{To: []string{"bob"}, Keys: []string{key}, IncludePermissions: true}, true); err != nil {
		t.Fatalf("runTrustSync: %v", err)
	}
	for _, want := range []string{
...
func TestTrustSyncFanOutAll(t *testing.T) {
	...
	if !strings.Contains(out.String(), `updated 1 project in "bob"`) || !strings.Contains(out.String(), `updated 1 project in "home". A running unmanaged claude`) {
		t.Errorf("fan-out output:\n%s", out.String())
	}
	if strings.Contains(out.String(), `"alice"`) && !strings.Contains(out.String(), `(from "alice")`) || strings.Contains(out.String(), `no changes for "alice"`) {
...
func TestTrustHint(t *testing.T) {
	...
	hint := trustHint(env.files, "bob", key)
	for _, want := range []string{
		`note: "bob" hasn't answered the folder-trust / external-imports dialogs for `,
		`("alice" has).`,
		"Carry them over:  bffs trust sync --to bob",
	} {
	...
	writeClaudeJSON(t, env.files["bob"], map[string]any{key: map[string]any{"hasTrustDialogAccepted": true}})
	if h := trustHint(env.files, "bob", key); !strings.Contains(h, "the external-imports dialog for") {
		t.Errorf("single-dialog hint = %q", h)
	}
	// Everything answered → no note. The source itself never gets one.
	writeClaudeJSON(t, env.files["bob"], map[string]any{key: acceptedEntry()})
	if h := trustHint(env.files, "bob", key); h != "" {
		t.Errorf("answered account got a hint: %q", h)
	}
	if h := trustHint(env.files, "alice", key); h != "" {
	...
	if h := trustHint(env.files, "bob", projectDir(t)); h != "" {
...
func TestCarryTrustOnLogin(t *testing.T) {
	...
	exists := projectDir(t)
	gone := filepath.Join(t.TempDir(), "gone")
	writeClaudeJSON(t, env.files["alice"], map[string]any{exists: acceptedEntry(), gone: acceptedEntry()})
	writeClaudeJSON(t, env.files["bob"], map[string]any{})

	n, src, err := carryTrustOnLogin(env.cfgDir, env.homeJSON, env.accs, "alice", "bob")
	if err != nil {
		t.Fatalf("carry: %v", err)
	}
	if n != 1 || src != "alice" {
		t.Errorf("n=%d src=%q, want 1 from alice", n, src)
	}
	got := readProjects(t, env.files["bob"])
	...
	if n, _, err := carryTrustOnLogin(env.cfgDir, env.homeJSON, env.accs, "alice", "bob"); err != nil || n != 0 {
		t.Errorf("re-carry: n=%d err=%v", n, err)
	}
	// No previous account, or the account itself → home is the source.
	if _, src, err := carryTrustOnLogin(env.cfgDir, env.homeJSON, env.accs, "", "bob"); err != nil || src != trust.HomeName {
		t.Errorf("no prevActive: src=%q err=%v", src, err)
	}
	if _, src, err := carryTrustOnLogin(env.cfgDir, env.homeJSON, env.accs, "bob", "bob"); err != nil || src != trust.HomeName {
		t.Errorf("self as prevActive: src=%q err=%v", src, err)
	}
	if _, src, err := carryTrustOnLogin(env.cfgDir, env.homeJSON, env.accs, "keyed", "bob"); err != nil || src != trust.HomeName {
...
func TestTrustBlockForAndLiveOwner(t *testing.T) {
	...
	live := []transcripts.LiveSession{
		{PID: 1, SessionID: "in", Cwd: filepath.Join(key, "sub"), ConfigDir: sessions.Dir(env.cfgDir, "alice")},
		{PID: 2, SessionID: "out", Cwd: t.TempDir(), ConfigDir: sessions.Dir(env.cfgDir, "alice")},
		{PID: 3, SessionID: "nocwd", ConfigDir: sessions.Dir(env.cfgDir, "alice")},
	}
	b, err := trustBlockFor(env, key, live)
	if err != nil {
		t.Fatal(err)
	}
	if len(b.Statuses) != 3 || b.Statuses[0].Account != "alice" || b.Statuses[2].Account != trust.HomeName {
	...
	// alice's own sessions/ dir is real (full isolation) → attributable.
	if err := os.MkdirAll(filepath.Join(sessions.Dir(env.cfgDir, "alice"), transcripts.RuntimeSessionsSubdir), 0o700); err != nil {
		t.Fatal(err)
	}
	if owner := liveOwner(env.files, live[0]); owner != "alice" {
		t.Errorf("owner = %q, want alice", owner)
	}
	if runtimeDirsCanShare() {
		// bob's sessions/ symlinked to alice's → shared, undeterminable.
		if err := os.MkdirAll(sessions.Dir(env.cfgDir, "bob"), 0o700); err != nil {
			t.Fatal(err)
		}
		if err := os.Symlink(filepath.Join(sessions.Dir(env.cfgDir, "alice"), transcripts.RuntimeSessionsSubdir), filepath.Join(sessions.Dir(env.cfgDir, "bob"), transcripts.RuntimeSessionsSubdir)); err != nil {
			t.Fatal(err)
		}
