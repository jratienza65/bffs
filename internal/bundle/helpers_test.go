...
const (
	...
	slugA    = "-Users-dev-build-projects-bffs"
...
func sampleManifest() (*Manifest, memOpener) {
	texts := map[string]string{
		"projects/" + slugA + "/" + sidA + ".jsonl": `{"type":"user","sessionId":"` + sidA + `","cwd":"/Users/dev/build/projects/bffs"}` + "\n" + `{"type":"assistant","message":{"usage":{"input_tokens":1}}}` + "\n",
	...
	m := &Manifest{
		...
		Created:       testNow,
		Source: Source{
			Hostname: "mac-a", User: "dev", Home: "/Users/dev", OS: "darwin", Arch: "arm64",
			ConfigDir: "/Users/dev/Library/Application Support/bffs/sessions/alice",
			RootDir:   "/Users/dev/.claude/projects", Account: "alice", AccountType: "oauth", Isolation: "partial",
		},
		Entries: []Entry{
			{
				Kind: EntrySession, Slug: slugA, Cwd: "/Users/dev/build/projects/bffs",
				ProjectKey: "/Users/dev/build/projects/bffs", GitRemote: "git@github.com:jratienza65/bffs.git",
				...
				Files: []File{
					fileFor("projects/"+slugA+"/"+sidA+".jsonl", src["projects/"+slugA+"/"+sidA+".jsonl"], testNow.Add(-30*time.Minute)),
					fileFor(longName, src[longName], testNow.Add(-31*time.Minute).Add(123456789*time.Nanosecond)),
					fileFor("projects/"+slugA+"/"+sidA+"/custom-title.json", src["projects/"+slugA+"/"+sidA+"/custom-title.json"], testNow.Add(-29*time.Minute)),
					fileFor("file-history/"+sidA+"/deadbeefdeadbeef@v1", src["file-history/"+sidA+"/deadbeefdeadbeef@v1"], testNow.Add(-40*time.Minute)),
					fileFor("history/"+sidA+".jsonl", src["history/"+sidA+".jsonl"], testNow.Add(-28*time.Minute)),
					fileFor("plans/quirky-lemur.md", src["plans/quirky-lemur.md"], testNow.Add(-50*time.Minute)),
				},
			},
			{
				Kind: EntryMemory, Slug: slugA, Cwd: "/Users/dev/build/projects/bffs", ProjectKey: "/Users/dev/build/projects/bffs",
				Files: []File{
					fileFor("memory/"+slugA+"/MEMORY.md", src["memory/"+slugA+"/MEMORY.md"], testNow.Add(-24*time.Hour)),
					fileFor("memory/"+slugA+"/topic.md", src["memory/"+slugA+"/topic.md"], testNow.Add(-25*time.Hour)),
				},
			},
		},
	}
