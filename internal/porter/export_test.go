...
func TestBuildManifestShape(t *testing.T) {
	...
	s := m.Source
	if s.Hostname == "" || s.Account != "alice" || s.AccountType != "oauth" || s.Isolation != "partial" || s.ConfigDir != e.claudeDir || s.RootDir != e.root.Dir || s.OS != runtime.GOOS || s.Arch != runtime.GOARCH {
		t.Errorf("source = %+v", s)
	}
...
func TestIdentAndBundleID(t *testing.T) {
	...
	if got := ident("Dev' Mac\x1b[2J"); got != "DevMac2J" {
		t.Errorf("ident = %q", got)
	}
