...
func stageSession(t *testing.T, dir string) CommitRequest {
	...
	write(t, tr, `{"type":"user","cwd":"/Users/dev/build/projects/bffs","sessionId":"`+testSID+`"}`+"\n"+`{"type":"assistant"}`)
...
func TestCommitSessionHappyPath(t *testing.T) {
	...
	req.Stamp = []byte(`{"type":"relocated","sessionId":"` + testSID + `","relocatedCwd":"/home/dev/src/bffs"}` + "\n")
	req.History = [][]byte{[]byte(`{"display":"hi","project":"/Users/dev/build/projects/bffs","sessionId":"` + testSID + `","timestamp":1700000000000}`)}
	req.HistoryProject = "/home/dev/src/bffs"
	dest, cfg := openDest(t)
	...
	if !strings.Contains(hist, `"project":"/home/dev/src/bffs"`) {
