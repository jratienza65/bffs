...
func histLine(sid string, ts int64, display string) []byte {
	return []byte(`{"display":"` + display + `","pastedContents":{},"project":"/Users/dev/old","sessionId":"` + sid + `","timestamp":` + itoa(ts) + `}`)
}
...
func TestAppendHistory(t *testing.T) {
	...
	added, err := AppendHistory(path, testSID, lines, "/home/dev/src/bffs")
	if err != nil {
		t.Fatal(err)
	}
	if added != 2 {
	...
	want := `{"display":"new one","pastedContents":{},"project":"/home/dev/src/bffs","sessionId":"` + testSID + `","timestamp":3000}` + "\n" +
		`{"display":"extra","project":"/home/dev/src/bffs","sessionId":"` + testSID + `","timestamp":6000}` + "\n"
	if tail != want {
		t.Errorf("appended:\n%s\nwant:\n%s", tail, want)
	}
...
func TestAppendHistoryCreatesFileAndKeepsProject(t *testing.T) {
	...
	if got := readFile(t, path); !strings.Contains(got, `"project":"/Users/dev/old"`) {
