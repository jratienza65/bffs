...
const (
	notesSrc = "---\nname: notes\npinned: true\n---\n# Notes\nsee /Users/dev/build/projects/bffs/x.md\n"
	logSrc   = "---\npinned: \"true\"\n---\nlog line\n"
...
func memDst(t *testing.T) string {
	return filepath.Join(t.TempDir(), "projects", "-home-dev-src-bffs", "memory")
}
...
func TestMergeMemorySkipAbsentConfirmed(t *testing.T) {
	...
	for _, r := range mv.Remaining {
		if r.File == "notes.md" && r.Path == "/Users/dev/build/projects/bffs/x.md" && r.Kind == transcripts.PathKindAbs {
			found = true
		}
	}
	if !found {
...
func TestMergeMemoryRefusesPlantedSymlink(t *testing.T) {
	...
	t.Run("slug dir", func(t *testing.T) {
		...
		slug := filepath.Join(pool, "-home-dev-src-bffs")
