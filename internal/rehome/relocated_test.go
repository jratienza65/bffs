...
func TestRelocatedRecord(t *testing.T) {
	got := string(RelocatedRecord(testSID, "/home/dev/src/bffs"))
	want := `{"type":"relocated","sessionId":"` + testSID + `","relocatedCwd":"/home/dev/src/bffs"}` + "\n"
	if got != want {
		t.Errorf("RelocatedRecord = %q, want %q", got, want)
	}
	// A Windows path is escaped, never mangled.
	got = string(RelocatedRecord(testSID, `C:\Users\dev\x`))
	if !strings.Contains(got, `"relocatedCwd":"C:\\Users\\dev\\x"`) {
