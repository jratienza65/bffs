...
func TestRowsSanitize(t *testing.T) {
	...
	s := transcripts.Session{ID: "1e005053-380a-4245-a145-52c2715afa73", Title: osc52 + "Plan: session export", GitBranch: osc52 + "main", Account: "alice", Size: 42_600_000, LastTS: now().Add(-2 * time.Hour)}
	r := &sessionRow{s: s, resolved: true, sel: map[string]bool{}, now: now}
	line := r.render(120)
	for _, want := range []string{"Plan: session export", "alice", "2h ago", "42.6 MB", "main"} {
	...
	narrow := r.render(50)
	if !strings.Contains(narrow, "Plan: session export") || strings.Contains(narrow, "alice") {
...
func TestScanPathLines(t *testing.T) {
	...
	if lines[0] != "notes.md:3: /Users/dev/x" || lines[1] != "@ref MEMORY.md:1: @~/notes.md" {
...
func TestRowsFillWidth(t *testing.T) {
	now := func() time.Time { return time.Date(2026, 8, 24, 12, 0, 0, 0, time.UTC) }
	rows := []row{
		&sessionRow{s: transcripts.Session{ID: "1e005053-380a-4245-a145-52c2715afa73", Title: "Plan", Account: "alice", GitBranch: "main", LastTS: now().Add(-time.Hour)}, resolved: true, sel: map[string]bool{}, now: now},
