...
// keys is the key map in force. It is built rather than declared
// because its help text carries glyphs, and the glyph set is resolved
// at start-up (glyphs.go) — a map built at package init would keep the
// symbols BFFS_ASCII asked it to drop.
var keys = newKeyMap()

func newKeyMap() keyMap {
	return keyMap{
		Up:          key.NewBinding(key.WithKeys("up", "k"), key.WithHelp(glyph.up+"/k", "up")),
		Down:        key.NewBinding(key.WithKeys("down", "j"), key.WithHelp(glyph.down+"/j", "down")),
...
var reservedHint = "(not available here; d never deletes " + glyph.emdash + " use bffs sessions rm)"
